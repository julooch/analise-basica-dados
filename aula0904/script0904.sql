...
select (max(salario) - min(salario)) as interseccao_renda from empregado where sexo='F';
select (max(salario) - min(salario)) as interseccao_renda from empregado where sexo='M';
...
select nome as empregado, salario as renda, format((salario * 0.12), 0.2) as inss, format((salario * 0.15), 0.2) as irpf from empregado; 
select char_length(nome) as tamanho_nome, (char_length(nome) + (rand() * char_length(nome)) * cod_emp) as id_aleatorio from empregado;
select 
	replace(nome, "A", "") as nome_modifcado, 
